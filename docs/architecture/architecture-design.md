...
估算假设：从当前空工程骨架起步；核心团队为 1 名技术负责人、5 名后端/Agent 工程师、2 名前端、2 名 SRE/平台工程师和 2 名 QA/安全工程师；另有产品负责人、安全、DBA 各 0.5 人按门禁参与；两周迭代；企业提供三故障域 Kubernetes、温备地域、OIDC 与企业微信/飞书测试租户。按此配置，首个生产试点预计 **38–48 个日历周、320–420 人周**；外部环境或合规评审延期不包含在区间内。

| 阶段 | 工期 / 工作量 | 主要依赖 | 阶段门禁与退出标准 |
|---|---:|---|---|
| M0 契约与工程基线 | 3–4 周 / 20–28 人周 | 架构、威胁与测试评审人到位 | 术语、ADR、API/事件/数据边界和公开测试接缝获接受，无未决架构冲突 |
| M1 控制面纵向切片 | 6–8 周 / 48–65 人周 | OIDC 测试租户、PostgreSQL | 租户、RBAC、Draft→Release→Deployment 可通过 Admin API 和 Web Console 完成并审计 |
| M2 数据面纵向切片 | 9–11 周 / 78–100 人周 | Kafka、Fake 模型/IM、M1 Release 契约 | 一条入站消息可跨 Gateway/Worker/Runner 完成；重复、乱序、重平衡不重复提交或执行副作用 |
| M3 治理与数据能力 | 8–10 周 / 72–94 人周 | M2 事件和 Session 模型、OPA/Vault | Tool 审批恢复、预算、数据分级、Memory/Knowledge、存储迁移通过隔离和一致性测试 |
| M4 生产化与连续性 | 7–9 周 / 60–78 人周 | 三故障域集群、温备地域、M2/M3 | GitOps、mTLS、审计删除、可观测性、备份恢复就绪；安全和供应链门禁无阻断项 |
| M5 容量、灾备与试点 | 5–6 周 / 42–55 人周 | 企业 IM、模型及审批人可用 | 达成容量/SLO 基线，Chaos 与独立温备演练满足 RPO/RTO，试点租户完成回滚和运维演练 |

关键路径为：租户与领域契约 → Release 契约 → 入站幂等/Outbox → Session 并发与事件权威 → Tool/预算/策略治理 → 生产发布门禁 → 多可用区及温备验证。Web Console 页面、Adapter 实现、可观测性面板和运行手册可在接口稳定后并行，但不能绕过对应阶段门禁。若 OIDC/IM 凭据、三故障域集群、温备地域、Vault/OPA 或真实模型端点晚于依赖点提供，里程碑等量顺延。最终退出要求是代码、契约、测试、图表、部署资产和运行手册一致，且不存在跨租户泄露、重复高风险副作用、秘密泄漏、迁移不一致或灾备目标失败等发布阻断项（[ADR-0036](../adr/0036-production-release-quality-gates.md)、[ADR-0044](../adr/0044-mandatory-agent-release-evaluation-gates.md)）。
